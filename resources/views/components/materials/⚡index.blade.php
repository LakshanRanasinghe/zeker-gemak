...
new class extends Component {
    ...
    public function startExport(?array $ids = null, bool $all = false, string $locale = 'en', string $headingLocale = 'en'): void
    {
        ...
        if ($query->count() === 0) {
            Flux::toast(__('No materials to export.'), variant: 'warning');
            return;
        }
        ...
        ExportMaterialsJob::dispatch($export);
...
<div @if ($activeExport && $activeExport->isInProgress()) wire:poll.2s="pollExport" @endif>
    <div class="flex items-center justify-between mb-6">
        <div>
            ...
            <flux:subheading size="lg" class="mb-6">{{ __('Manage your materials.') }}
            </flux:subheading>
        </div>
        <div class="flex gap-2">
            <flux:button variant="primary" icon="plus" href="{{ route('materials.create') }}" wire:navigate>
                {{ __('New Material') }}
            </flux:button>
        </div>
    </div>
