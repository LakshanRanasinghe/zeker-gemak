...
<head>
    ...
    <title>{{ __('order_emails.placed.title_tag') }}</title>
    <style>
    ...
    <div class="card">
        <div class="header">
            <div class="header-inner">
                ...
                <h1>{{ __('order_emails.placed.heading') }}</h1>
                <p>{{ __('order_emails.placed.intro') }}</p>
                <span class="order-pill">{{ __('order_emails.placed.order_number', ['number' => $order->number]) }}</span>
            </div>
        </div>

        <div class="banner">
            <p>
                {{ __('order_emails.placed.greeting', ['name' => $order->billpayer?->firstname ?? __('order_emails.placed.fallback_name')]) }}
                {{ __('order_emails.placed.summary_intro') }}
            </p>
        </div>

        <div class="section">
            <p class="section-title">{{ __('order_emails.placed.order_summary') }}</p>
            <div class="row"><span class="label">{{ __('order_emails.placed.order_number_label') }}</span><span class="value">#{{ $order->number }}</span></div>
            <div class="row"><span class="label">{{ __('order_emails.placed.order_date') }}</span><span class="value">{{ $order->created_at->format(__('order_emails.placed.date_format')) }}</span></div>
            <div class="row"><span class="label">{{ __('order_emails.placed.payment_status') }}</span><span class="value">{{ __('order_emails.status.' . $order->status->value()) }}</span></div>
        </div>

        <div class="section">
            <p class="section-title">{{ __('order_emails.placed.items_ordered') }}</p>
            <table class="items">
                <thead>
                    <tr>
                        <th>{{ __('order_emails.placed.product') }}</th>
                        <th style="text-align:center">{{ __('order_emails.placed.quantity') }}</th>
                        <th style="text-align:right">{{ __('order_emails.placed.price') }}</th>
                        <th style="text-align:right">{{ __('order_emails.placed.total') }}</th>
                    </tr>
                </thead>
                <tbody>
                            ...
                            <td>
                                ...
                                @if(data_get($item->configuration, 'type') === 'extended_warranty')
                                    <span class="item-meta">
                                        {{ __('order_emails.placed.extended_warranty_for', ['product' => data_get($item->configuration, 'parent_name')]) }}
                                        @if(data_get($item->configuration, 'duration_months'))
                                            <br>{{ __('order_emails.placed.warranty_duration', ['months' => data_get($item->configuration, 'duration_months')]) }}
                                        @endif
                                        @if(data_get($item->configuration, 'sku'))
                                            <br>{{ __('order_emails.placed.warranty_sku', ['sku' => data_get($item->configuration, 'sku')]) }}
                                        @endif
                                    </span>
                                @endif
                            </td>
                ...
                <tfoot>
                    <tr>
                        <td colspan="3" style="text-align:right; font-weight:normal; color:#78716c;">{{ __('order_emails.placed.subtotal') }}</td>
                    ...
                    @endforeach
                    <tr>
                        <td colspan="3" style="text-align:right; font-weight:normal; color:#78716c;">{{ __('order_emails.placed.shipping') }}</td>
                    ...
                    <tr>
                        <td colspan="3" style="text-align:right; font-weight:normal; color:#78716c;">{{ __('order_emails.placed.tax') }}</td>
                    ...
                    <tr>
                        <td colspan="3" style="text-align:right; font-weight:700; font-size:16px; color:#ea580c;">{{ __('order_emails.placed.order_total') }}</td>
        ...
        @if($order->shippingAddress)
            <div class="section">
                <p class="section-title">{{ __('order_emails.placed.shipping_to') }}</p>
                <div class="address-box">
                    <strong>{{ __('order_emails.placed.delivery_address') }}</strong>
        ...
        @if($order->notes)
            <div class="section">
                <p class="section-title">{{ __('order_emails.placed.order_notes') }}</p>
