...
<head>
  ...
  <title>New Recycle Request — {{ config('app.company.name') }}</title>
</head>
        ...
        <tr>
          <td style="background-color:#f08500;padding:20px 32px;">
            <p style="margin:0;font-size:20px;font-weight:700;color:#ffffff;letter-spacing:-0.01em;">
              {{ $data['type'] === 'pickup' ? 'Recycle Pickup Request' : 'Recycle Box Request' }}
            </p>
            <p style="margin:5px 0 0;font-size:13px;color:#fff3e0;line-height:1.5;">
              A customer has submitted a new recycling request on the Epson ColorWorks Recycle page.
            </p>
          </td>
        </tr>
            ...
            <table width="100%" cellpadding="0" cellspacing="0"
              ...
              @php
                $infoRows = [
                  ...
                  ['label' => 'Street / House No.', 'value' => $data['street'] ?? ''],
                  ['label' => 'Postal Code', 'value' => $data['postal'] ?? ''],
                  ['label' => 'City', 'value' => $data['city'] ?? ''],
                  ['label' => 'Address Line 2', 'value' => $data['extra'] ?? ''],
                ];
                
                if ($data['type'] === 'pickup') {
                    $infoRows[] = [
                        'label' => 'New Box Needed?',
                        'value' => !empty($data['newBox']) ? 'Yes' : 'No'
                    ];
                }

                $infoRows = array_filter($infoRows, fn($row) => $row['value'] !== '' && $row['value'] !== null);
                $i = 0;
              @endphp
              ...
              <tr>
                <td style="padding:16px 20px;">
                  ...
                  <p style="margin:6px 0 0;font-size:13px;color:#7c2d12;line-height:1.6;">
                    Please process this request. You can contact the customer at 
                    <a href="mailto:{{ $data['email'] ?? '' }}" style="color:#f08500;font-weight:600;">{{ $data['email'] ?? '' }}</a> or call them at {{ $data['phone'] ?? '' }}.
                  </p>
                </td>
              </tr>
            </table>
              ...
              <tr>
                <td style="vertical-align:top;">
                  ...
                  <p style="margin:5px 0 0;font-size:12px;color:#475569;line-height:1.6;">
                    <a href="mailto:{{ config('app.company.email') }}"
                      style="color:#64748b;text-decoration:none;">{{ config('app.company.email') }}</a>
                    &nbsp;&middot;&nbsp;
                ...
                <td align="right" style="vertical-align:top;">
                  <p style="margin:0;font-size:11px;color:#334155;line-height:1.5;">
                    Auto-generated by<br/>{{ config('app.company.website') }}
                  </p>
                </td>
              </tr>
            </table>
