...
new class extends Component {
    ...
    public string $itemNamePlural = 'categories';
    ...
    public function mount(string $taxonomyName = 'Category', string $itemName = 'Category', ?string $itemNamePlural = null)
    {
        $this->taxonomyName = $taxonomyName;
        $this->itemName = $itemName;
        $this->itemNamePlural = $itemNamePlural ?: Str::plural(Str::lower($itemName));
...
<div class="space-y-4" x-data="{
    ...
    <div class="px-1">
        <flux:input x-model="search" placeholder="{{ __('Search :items...', ['items' => $itemNamePlural]) }}" icon="magnifying-glass" size="sm"
            clearable />
    </div>
    ...
    <div
        ...
        @if($this->tree->isEmpty())
            <div class="py-4 text-center">
                <flux:subheading>{{ __('No :items found.', ['items' => $itemNamePlural]) }}</flux:subheading>
            </div>
        @else
            <div class="space-y-1">
    ...
    <div>
        @if(!$showAddForm)
            <flux:button variant="subtle" size="sm" icon="plus" wire:click="$set('showAddForm', true)" class="text-xs">
                {{ __('Add New :item', ['item' => $itemName]) }}
            </flux:button>
        @else
            <div
                class="space-y-3 bg-white dark:bg-zinc-800 p-3 rounded-lg border border-zinc-200 dark:border-zinc-700 shadow-sm">
                <flux:input wire:model="newCategoryName" placeholder="{{ __(':item name...', ['item' => $itemName]) }}" size="sm" />

                <flux:select wire:model="parentId" placeholder="{{ __('Parent :item (optional)', ['item' => $itemName]) }}" size="sm">
