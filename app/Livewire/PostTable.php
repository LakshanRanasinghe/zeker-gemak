...
{
    public string $tableName = 'posts-table';
    ...
    public function datasource(): Builder
    {
        ...
        return DB::table('posts')
            ...
            ->selectRaw("
                posts.id,
                COALESCE(t.name, JSON_UNQUOTE(JSON_EXTRACT(t.fields, '$.title')), posts.title) as title,
                COALESCE(t.slug, JSON_UNQUOTE(JSON_EXTRACT(t.fields, '$.slug')), posts.slug) as slug,
                posts.author,
                posts.post_type,
                posts.status,
                posts.created_at
    ...
    public function header(): array
    {
        ...
        if (! empty($this->checkboxValues)) {
            $buttons[] = Button::add('bulk-delete')
                ...
                ->attributes([
                    'x-data' => '',
                    'x-on:click' => "if(confirm('".__('Are you sure you want to delete the selected posts?')."')) { \$wire.bulkDelete() }",
                ]);
        }
    ...
    public function actions(object $row): array
    {
        return [
            Button::add('edit')
                ->slot(__('Edit'))
                ->class('px-2 py-1 bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-md text-sm shadow-sm hover:bg-zinc-50 dark:hover:bg-zinc-700 text-zinc-700 dark:text-zinc-300 transition-colors')
                ->route('posts.edit', ['post' => $row->id])
                ->attributes(['wire:navigate' => '']),

            Button::add('delete')
                ->slot(__('Delete'))
                ->class('px-2 py-1 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-md text-sm shadow-sm hover:bg-red-100 dark:hover:bg-red-900/40 text-red-600 dark:text-red-400 transition-colors')
                ->dispatch('deletePost', ['id' => $row->id])
                ->confirm(__('Are you sure you want to delete this post? This action cannot be undone.')),
        ];
    }
    ...
    public function bulkDelete(): void
    {
        ...
        if (empty($ids)) {
            Flux::toast(__('Please select at least one post.'), variant: 'warning');

            return;
        }

        Post::whereIn('id', $ids)->where('post_type', 'post')->get()->each->delete();

        $this->checkboxValues = [];
        $this->checkboxAll = false;
        $this->dispatch('pgBulkActions::clear', $this->tableName);

        Flux::toast(__('Selected posts deleted successfully.'), variant: 'success');
    }
    ...
    public function deletePost(int $id): void
    {
        ...
        Flux::toast(__('Post deleted successfully.'), variant: 'success');
    }
}
